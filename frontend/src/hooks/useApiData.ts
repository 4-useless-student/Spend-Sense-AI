...
export function useApiData<T>(
  ...
  const refreshEventsKey = options.refreshEvents?.join("|") ?? "";

  useEffect(() => {
    if (!options.refreshEvents?.length) return;
    const handleRefresh = () => reload();
    options.refreshEvents.forEach((eventName) => window.addEventListener(eventName, handleRefresh));
    return () => {
      options.refreshEvents?.forEach((eventName) => window.removeEventListener(eventName, handleRefresh));
    };
  }, [refreshEventsKey, reload]);
  ...
  useEffect(() => {
    ...
    setLoading((current) => current || !hasData);
