...
| **Application Server** | `«container : Docker»` | `uvicorn` (ASGI) cổng **8080** chạy FastAPI + Pipeline + JWT Auth. Production dùng Gemini Vision cho phân tích hóa đơn; YOLOv11/VietOCR là legacy mode. | Lõi xử lý của hệ thống: nhận request REST, điều phối pipeline phân tích hóa đơn, sinh insight, xác thực. |
...
- **Receipt analysis:** Production mặc định dùng **Gemini Vision** (`RECEIPT_ANALYZER=gemini_vision`) để tránh tải YOLO/VietOCR trong container RAM thấp. Legacy mode `yolo_vietocr` vẫn có sẵn cho môi trường mạnh hơn.
- **OCR engine legacy:** OCR cũ được hiện thực bằng **VietOCR** (`vgg_seq2seq`), tối ưu cho tiếng Việt (`src/vision/ocr.py`).
